...
class PlayerProxy(object):
	...
	def __init__(self, avt_mb, owner, idx):
		...
		# 明杠次数
		self.exposed_kong = 0
		# 每局 杠信息, 为了结算, 只有赢得人才有效
		self.kong_num_list = [0, 0, 0] # concealedKong exposedKong self_exposedKong
	...
	def concealedKong(self, tile):
		...
		# 算接杠和放杠的分
		# 赢家结算有效 self.owner.cal_score(self.idx, const.OP_CONCEALED_KONG)
		self.kong_num_list[0] += 1
	...
	def exposedKong(self, tile):
		...
		# 算接杠和放杠的分
		# 赢家结算有效 self.owner.cal_score(self.idx, const.OP_GET_KONG)
		self.kong_num_list[1] += 1
		# 不放杠
	...
	def self_exposedKong(self, tile):
		...
		# if not win_list:
		# 明杠得分, 只有不被抢杠胡才能得分
		# 赢家结算有效self.owner.cal_score(self.idx, const.OP_EXPOSED_KONG)
		self.kong_num_list[2] += 1
		self.owner.current_idx = self.idx
		self.owner.cutAfterKong()
		self.owner.beginRound()
		# else:
		# 	self.owner.processKongWin(win_list, tile)
